...
import { Edit3, Trash2, Plus, GripVertical, ChevronUp, ChevronDown, Check, X, Clock, Star } from 'lucide-react'
...
export function SubtaskEditor({ subtasks, onConfirm, onBack, onRegenerate }: Props) {
  ...
  const handleStartEdit = (item: Subtask) => {
  ...
  const handleAdd = () => {
    ...
    const item: Subtask = {
      ...
      isBoss: false,
      unlocked: true,
      points: DIFFICULTY_CONFIG[newDifficulty].points,
    }
    setItems([...items, item])
    setNewTitle('')
    setShowAdd(false)
    ...
    <div className="max-w-lg mx-auto">
          ...
          {items.map((item, idx) => {
            ...
            return (
              <motion.div
                ...
                exit={{ opacity: 0, x: 10 }}
                className={`flex items-center gap-2 p-3 rounded-xl border ${diff.bg} transition-colors`}
              >
                {/* Drag handle */}
                <div className="flex flex-col gap-0.5 shrink-0">
                  <button onClick={() => handleMoveUp(idx)} className="p-0.5 hover:bg-secondary/50 rounded">
                    <ChevronUp className="w-3 h-3 text-muted-foreground" />
                  </button>
                  <button onClick={() => handleMoveDown(idx)} className="p-0.5 hover:bg-secondary/50 rounded">
                  ...
                  {editingId === item.id ? (
                    <div className="flex gap-2">
                      <input
                        ...
                        className="flex-1 px-2 py-1 text-sm rounded bg-secondary border border-border focus:border-primary focus:outline-none"
                        autoFocus
                      ...
                      <input
                        ...
                        className="w-16 px-2 py-1 text-sm rounded bg-secondary border border-border focus:border-primary focus:outline-none"
                        min={5}
                        max={60}
                      />
                      <button onClick={handleSaveEdit} className="p-1 text-emerald-400 hover:bg-secondary rounded">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${diff.bg} ${diff.color}`}>
                        {diff.label}
                      </span>
                      {item.isBoss && (
      ...
      {showAdd ? (
        <motion.div
          ...
          className="mb-4 p-3 rounded-xl border border-dashed border-primary/50 bg-primary/5"
        >
          <div className="flex gap-2 mb-2">
            <input
              ...
              placeholder="新关卡名称..."
              className="flex-1 px-3 py-2 text-sm rounded-lg bg-secondary border border-border focus:border-primary focus:outline-none"
              autoFocus
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-4 py-2 border border-border rounded-lg text-sm text-muted-foreground hover:bg-secondary"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
