...
import { Sword, Plus, Sun, Moon, Brain, Store, Coins } from 'lucide-react'
...
export default function App() {
  ...
  const handleCompleteSubtask = (taskId: string, subtaskId: string) => {
    setTasks(prev => {
      const updated = prev.map(t => {
        ...
        const pointsEarned = (t.pointsEarned || 0) + (completedSub?.points || 10)

        return {
          ...t,
          subtasks: subs,
          totalCompleted: completedCount,
          bossDefeated: allDone,
          pointsEarned,
        }
      })
      saveTasks(updated)
  ...
  const handleStartBoss = (taskId: string) => {
    setBossTaskId(taskId)
  ...
  const handleBossDefeated = () => {
    ...
    setTasks(prev => {
      const updated = prev.map(t => {
        ...
        addPoints(bossPoints + (t.pointsEarned || 0))
        return {
          ...t,
          bossDefeated: true,
          pointsEarned: (t.pointsEarned || 0) + bossPoints,
          subtasks: t.subtasks.map(s => s.isBoss ? { ...s, completed: true } : s),
        }
      })
      saveTasks(updated)
      return updated
  ...
  const handleSlackOff = (task: Task) => {
    setNegotiatingTask(task)
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 ${darkMode ? '' : 'light'}`}>
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'edit_subtasks' && pendingTaskInfo && (
            ...
            >
              <SubtaskEditor
                ...
                onRegenerate={async () => {
                  const newSubs = await decomposeTask(
                    pendingTaskInfo.name,
                    pendingTaskInfo.deadline,
                    personality || 'avoider'
                  )
          ...
          {showCapsule && activeTask && (
            <TimeCapsuleModal
              ...
              onSkip={() => { setShowCapsule(false); setPhase('task_board') }}
            />
          )}
            ...
            >
              ...
              {tasks.length === 0 ? (
                <div className="text-center py-20">
                  ...
                  <button
                    onClick={() => setPhase('future_mirror')}
                ...
                <div className="space-y-6">
                  {tasks.map(task => (
                    <TaskBoard
                      ...
                      onSlackOff={() => handleSlackOff(task)}
          ...
          {phase === 'boss_battle' && bossTaskId && (
            <BossBattle
              task={tasks.find(t => t.id === bossTaskId)!}
      ...
      {showNegotiate && negotiatingTask && (
        <NegotiateDialog
          taskName={negotiatingTask.name}
          personality={negotiatingTask.personality || personality || 'avoider'}
          onStay={() => setShowNegotiate(false)}
          onClose={() => setShowNegotiate(false)}
        />
      )}

      {showFlowBreak && (
        <FlowBreak onClose={() => setShowFlowBreak(false)} />
      )}

      {showFuneral && (
        <TaskFuneral task={showFuneral} onClose={() => setShowFuneral(null)} />
      )}
