      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        <AnimatePresence mode="wait">
          ...
          {phase === 'task_board' && <motion.div key="t" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
            ...
            <div className="flex gap-1 mb-4 p-1 rounded-xl bg-secondary/50">{(['all','todo','done'] as const).map(f => <button key={f} onClick={() => setTaskFilter(f)} className={'flex-1 py-2 rounded-lg text-sm font-medium transition-all ' + (taskFilter === f ? 'bg-card shadow-sm text-foreground' : 'text-muted-foreground hover:text-foreground')}>{f === 'all' ? '全部' : f === 'todo' ? '待办' : '已完成'}{f === 'todo' && <span className="ml-1.5 text-xs px-1.5 py-0.5 rounded-full bg-primary/20 text-primary">{tasks.filter(t => !t.bossDefeated).length}</span>}</button>)}</div>
            {tasks.length === 0 ? <div className="text-center py-20"><motion.div animate={{ y: [0, -8, 0] }} transition={{ duration: 3, repeat: Infinity }}><Brain className="w-20 h-20 mx-auto mb-4 text-muted-foreground/30" /></motion.div><p className="text-muted-foreground mb-2 text-lg font-medium">还没有任务</p><p className="text-muted-foreground/60 mb-6 text-sm">来拆解第一个巨型任务吧！</p><button onClick={() => setPhase('future_mirror')} className="inline-flex items-center gap-2 px-8 py-3.5 bg-gradient-to-r from-primary to-red-500 text-white rounded-2xl font-bold text-lg hover:shadow-lg hover:shadow-primary/25 transition-all"><Plus className="w-5 h-5" />新建任务</button></div>
            : <div className="space-y-6">{getFilteredTasks().map(task => <TaskBoard key={task.id} task={task} onCompleteSubtask={handleCompleteSubtask} onStartBoss={handleStartBoss} onSlackOff={() => handleSlackOff(task)} onFuneral={task.bossDefeated ? () => setShowFuneral(task) : undefined} onDeleteTask={() => handleDeleteTask(task.id)} onUndoSubtask={(sid: string) => handleUndoSubtask(task.id, sid)} onStartPomodoro={(sid: string) => handleStartPomodoro(task, sid)} />)}<div className="text-center pt-4"><button onClick={() => setPhase('future_mirror')} className="inline-flex items-center gap-2 px-5 py-2.5 border border-border rounded-xl hover:bg-secondary hover:border-primary/30 transition-all text-sm"><Plus className="w-4 h-4" />添加新任务</button></div></div>}
          </motion.div>}
